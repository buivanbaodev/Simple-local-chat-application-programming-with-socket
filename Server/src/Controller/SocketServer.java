...
public class SocketServer implements Runnable {
    ...
    public SocketServer(ServerFrame frame, int Port){
       //Số client
        clients = new ServerThread[ServerConfig.MAX_User];
        ui = frame;
        port = Port;
        db = new Database();
	...
	try{  
	    server = new ServerSocket(port);
            port = server.getLocalPort();
	    ui.txtServer.append("Bat Dau Ket Noi Server. IP : " + InetAddress.getLocalHost() + ", Port: " + server.getLocalPort());
	    start(); 
        }
	catch(IOException ioe){  
            ui.txtServer.append("\nKhong The Tim Thay port " + port + ": " + ioe.getMessage()); 
	}
    }
